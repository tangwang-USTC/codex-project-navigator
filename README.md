...
9. “项目”根目录、项目和任意分组都可右键新建：根目录可新建一级分组或一级任务，后两者可新建下级分组、添加已有任务、从本机任务文件夹添加或发起新任务。根目录新建的一级分组是无工作目录的 Navigator 容器。
10. “在官方 Codex 中新建任务…”只登记 Navigator 目标位置并打开官方 Codex 侧栏；请在官方界面点击“新建 Codex Agent”并发送首条消息。任务被官方服务真实创建后，Navigator 会自动归入原项目/分组，不会产生空任务或调用语义拆分流程。
11. 分组支持任意深度；“未分组”不再是自动节点，而是可按需创建的普通分组名称。删除任意分组后，其中任务回到父层级。
...
分组层级按每个项目自身数据决定，不设全局层级开关：没有分组的项目直接显示任务；创建分组后自动呈现所需的三层、四层或更深结构。
